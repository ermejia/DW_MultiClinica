...
export class EditPatientRoomComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private router: Router, private patientRoomService: DataService,
  ...
  ngOnInit(){
    this.patientRoomService.getPatientRoom(this.route.snapshot.params.id).subscribe((result)=>{
  ...
  save() {
    this.updatePatientRoom
      .updatePatient(this.patientRoom).subscribe(data => {
        this.gotoList();
      });
